...
function CreateProfile() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await createProfileMutation(formData).unwrap();
      showAlert(dispatch, "Profile Updated!");
      navigate("/dashboard");
    } catch (err) {
      dispatch(
        setAlert({
          msg: err?.data?.message || "Failed to create profile",
          type: "danger",
        }),
      );
    }
  ...
  return (
    <>
      ...
      <form className="form" onSubmit={onSubmit}>
        ...
        <div className="my-2">
          <button
            onClick={() => toggleSocialInputs((b) => !b)}
            type="button"
        ...
        >
          {isCreatingProfile ? "Creating..." : "Submit"}
        </button>
